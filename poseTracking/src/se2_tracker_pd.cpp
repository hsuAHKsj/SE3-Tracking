...
int main(int argc, char** argv)
{
    ...
    if (se2_pathgen(interpolated) == EXIT_SUCCESS)
    {
        ...
        manif::SE2Tangentd se2_vel_command_t_1 = manif::SE2Tangentd::Zero();
        ...
        for (int i = 0; i < interpolated.size() - 1; i++)
        {
            ...
            // 计算目标加速度
            manif::SE2Tangentd tar_acc = (tar_vel - se2_vel_command_t_1) / delta_T;
            if (se2_vel_command_t_1.isApprox(manif::SE2Tangentd::Zero()))
            {
                manif::SE2Tangentd tar_acc = manif::SE2Tangentd::Zero();
            }
            
            //manif::SE2Tangentd delta_se_vel = se2_error * 0.5;

            // 计算控制率产生的指令加速度
            manif::SE2Tangentd se2_acc_cLaw = tar_pose.rminus(se2_pos)* k_p + (tar_vel - se2_vel_t_1)* k_d + tar_acc;


            // 对角速度，角加速度进行约束
            double rot_vec_bound = 0.6;
            double rot_acc_bound = 3.5;

            // 如果加速度过大，对整个矢量进行截断
            manif::SE2Tangentd applied_vel = se2_vel_t_1 + se2_acc_cLaw*delta_T;
            if (fabs(se2_acc_cLaw.angle()) > rot_acc_bound)
            {
                manif::SE2Tangentd applied_acc = se2_acc_cLaw / fabs(se2_acc_cLaw.angle()) * rot_acc_bound;
