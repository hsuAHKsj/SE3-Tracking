...
void data_interpreter_state_keeper(const imuDataPack& iData, manif::SE3d& pose, Eigen::Vector3d& ang_d, Eigen::Vector3d& ang_dd, Eigen::Vector3d& real_ang_d)
{
    ...
    Eigen::Vector3d eular_d(iData.w[0], iData.w[1], iData.w[2]);
    real_ang_d = poseEi * eular_d / 180 * PI;
...
int main()
{
    ...
    std::vector<double> angel_vel_imu_v;
    ...
    do
    {
        ...
        m.unlock();
        // 从内存池中获取
        data_interpreter_state_keeper(temImuData, imuInitPose, ang_d_diff, ang_dd_diff, ang_d_imu);
    ...
    while (iter++ < RECORD_TIME)
    {
        ...
        data_interpreter_state_keeper(iData_temp, tar_pose, ang_d_diff, ang_dd_diff, ang_d_imu);

        //data_interpreter(iData_temp, tar_pose, ang_d, ang_dd);
        angel_vel_diff_v.push_back(ang_d_diff.norm());
        angel_vel_imu_v.push_back(ang_d_imu.norm());
        ...
        manif::SE3Tangentd se3_vel_cLaw = k_p * tar_pose.rminus(roboPoseSE3) + k_v * (angel_d) + k_a * angel_dd;
    ...
    // 画出角速度
    std::string angleVel_diff = "angleVel_Diff";
    saveTime(angleVel_diff, angel_vel_diff_v);

    // 保存真实角速度
    std::string angleVel = "angleVel";
    saveTime(angleVel, angel_vel_imu_v);
